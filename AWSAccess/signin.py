...
class AWSConnection(Connection):
    ...
    def __init__(self, host, region, **kwargs):
        ...
        self.host = host
        self.region = region
        self.token = kwargs['session_token'] if 'session_token' in kwargs \
                                             else os.environ.get('AWS_SESSION_TOKEN')
        self.secret = kwargs['secret_key'] if 'secret_key' in kwargs \
                                             else os.environ.get('AWS_SECRET_ACCESS_KEY')
        self.key = kwargs['access_key'] if 'access_key' in kwargs \
    ...
    def perform_request(self, method, url, params=None,
        ...
        client = ESConnection(region=self.region,
                              ...
                              is_secure=False)
        if body:
            response = client.make_request(method, path=url,
